...
**Why NOT S3/R2 Now**:
- Adds setup time (2 days until interview)
...
> "For the MVP, I used local filesystem storage with Docker volumes. This let me focus on the spell-checking algorithm without cloud setup overhead. The file operations are abstracted behind a service layer with a clear interface, so migrating to S3 or Cloudflare R2 in production would be straightforward—just swap the implementation without touching the API layer. This demonstrates MVP thinking: defer infrastructure optimization, focus on core value first."
